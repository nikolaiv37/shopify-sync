#!/usr/bin/env python3
"""Paginate all vendor:Europe variants and dump SKU/inventory info."""
import json
import sys
import time
...
QUERY = open("europe_variants.graphql").read()
...
while True:
    ...
    try:
        ...
        if consecutive_errors > 10:
            print(f"[page {page}] Too many errors, aborting. Last: {err[:400]}", file=sys.stderr)
            break
...
with open("europe_variants.json", "w", encoding="utf-8") as f:
    json.dump(variants, f, ensure_ascii=False)

print(f"DONE. {len(variants)} variants saved to europe_variants.json")
