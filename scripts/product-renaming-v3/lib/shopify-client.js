/**
 * Shopify Admin GraphQL client for product-renaming V3.
 *
 * Mutation surface is intentionally tiny:
 *   updateProductRename({ productId, newTitle, newSeoTitle, newSeoDescription, newDescriptionHtml })
 *
 * No setter is exposed for handle, sku, vendor, productType, tags, variants,
 * price, inventory, images, status, or collections. A V3 caller cannot mutate
 * any of these even by accident.
 */
...
export function createShopifyClient() {
  ...
  /**
   * Update only the V3 mutation surface. Anything not in the destructured
   * signature is intentionally unreachable. A separate productUpdate call is
   * issued for descriptionHtml so the title-mutation result and the
   * description-mutation result are both visible to the caller.
   */
  async function updateProductRename({
    productId, newTitle, newSeoTitle, newSeoDescription, newDescriptionHtml,
  }) {
