#!/usr/bin/env node
/**
 * Restore products from a V3 job's rollback.json.
 *
 * Default mode is DRY-RUN. Real mutations only with --apply.
 *
 * Usage:
 *   node scripts/product-renaming-v3/rollback-job.js --job=<jobId>
 *   node scripts/product-renaming-v3/rollback-job.js --job=<jobId> --apply --limit=10
 *
 ...
 * captured at apply time. Same mutation policy as apply — only title,
...
async function main() {
  ...
  if (!rollback?.rollbackItems || rollback.rollbackItems.length === 0) {
    console.log('No rollback items recorded for this V3 job. Nothing to do.');
    process.exit(0);
  }

  const mode = opts.apply ? 'APPLY' : 'DRY';
  console.log(`[${new Date().toISOString()}] V3 Rollback ${opts.job} mode=${mode}`);
  ...
  console.log();
  console.log('========== V3 Rollback summary ==========');
