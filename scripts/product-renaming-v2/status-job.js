#!/usr/bin/env node
/**
 * Show the current status of a V2 job (or list jobs).
 *
 * READ-ONLY. No Shopify calls.
 *
 * Usage:
 *   node scripts/product-renaming-v2/status-job.js --job=<jobId>
 *   node scripts/product-renaming-v2/status-job.js --list
 */
...
function hashLine(label, expected, current) {
  ...
  const marker = match ? 'MATCH' : 'MISMATCH';
  return pad(label, `${marker}  current=${current.slice(0, 12)}  recorded=${expected.slice(0, 12)}`);
}
...
async function showOne(jobId) {
  ...
  const skipped = planCounts.skipped || 0;
  ...
  console.log(pad('approveMedium', validation?.approveMedium == null ? '(no validation)' : String(validation.approveMedium)));
  console.log(pad('duplicate titles', validation?.duplicates?.length || 0));
  console.log(pad('hard blockers', validation?.hardBlockers ?? 0));
  console.log();
...
async function showList() {
  ...
  console.log();
  console.log('Jobs (most recent first):');
