#!/usr/bin/env node
/**
 * Export Shopify products into a V2 job folder.
 *
 * READ-ONLY. No mutations.
 *
 * Usage:
 *   node scripts/product-renaming-v2/export-job-products.js --job=<jobId>
 *   node scripts/product-renaming-v2/export-job-products.js --job=<jobId> --resume
 *
 * --resume: continue paginating from the cursor stored in export.json.
 *           Without --resume, the export restarts from the beginning.
 */
...
async function main() {
  ...
  for await (const page of client.paginateProducts({ query, startCursor: cursor })) {
    ...
    // Checkpoint every page so --resume can pick up.
  ...
  console.log('=====================================');
  console.log();
  console.log(`Next: npm run rename:v2:plan -- --job=${opts.job}`);
}
