#!/usr/bin/env node
/**
 * Create a new product-renaming V2 job.
 *
 * READ-ONLY. Does not touch Shopify.
 *
 * Usage:
 *   node scripts/product-renaming-v2/create-job.js --category=garden
 *   node scripts/product-renaming-v2/create-job.js --category=sofas --vendor=Europe --status=ACTIVE
 *   node scripts/product-renaming-v2/create-job.js --category=garden --query="tag:градински" --limit=500
 *
 * Writes:
 *   logs/product-renaming/jobs/<jobId>/job.json
 */
...
async function main() {
  ...
  const job = {
    ...
    mutationPolicy: {
      allowed: ['title', 'seo.title', 'seo.description', 'descriptionHtml(modelReplacement)'],
      forbidden: [
  ...
  console.log(`Category:   ${opts.category || '(auto-detect)'}`);
  ...
  console.log('==================================');
  console.log();
  console.log(`Next: npm run rename:v2:export -- --job=${jobId}`);
  console.log();

  // Print just the job id on the last line so callers can capture it.
