...
export function evaluateRow({ node, requestedSku, feedEntry, isFeedDuplicate, expectedVendor, sellingEffective, compareOperation, compareMultiplier }) {
  ...
  const targetSelling = computeTargetPrice(wholesale, sellingEffective);
  const compare = computeCompareTarget(compareOperation, compareMultiplier, wholesale);
  if (targetSelling == null || (compare.mode === 'set' && compare.value == null)) {
  ...
  const sellingChanged = liveSelling == null || round2(liveSelling) !== targetSelling;
  ...
  const compareWarn = compare.mode === 'set' && compare.value != null && round2(compare.value) <= targetSelling;
