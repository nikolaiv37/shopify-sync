 ...
 * @param {number|null} args.sellingEffective - selling operation multiplier (SOURCE = 1), null for KEEP
...
export function classifyPrices({
  ...
  const keepSelling = sellingMeta?.operation === 'keep' || sellingEffective == null;
  const rows = [];
  const counts = {
  ...
  for (const [sku, feed] of feedIndex.bySku) {
    ...
    const target = keepSelling || feed.wholesale == null ? null : computeTargetPrice(feed.wholesale, sellingEffective);
    ...
    // Invalid wholesale only blocks fields whose selected operation derives from wholesale.
    if ((!keepSelling && target == null) || (compare.mode === 'set' && compare.value == null)) {
      counts.invalidPrice++;
      rows.push({ ...enriched, status: PriceStatus.INVALID_PRICE, reason: 'invalid-wholesale' });
      continue;
    }

    const sellingChanged = keepSelling ? false : currentSelling == null || round2(currentSelling) !== target;
    ...
    const effectiveSelling = keepSelling ? currentSelling : target;
    ...
    const diff = keepSelling || currentSelling == null ? null : round2(target - currentSelling);
