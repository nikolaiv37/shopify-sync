...
function resolveSellingOperation({ operation, multiplier }) {
  ...
  if (op !== PriceOperation.SOURCE && op !== PriceOperation.MULTIPLIER) {
...
export async function previewPrices({ supplierKey, selling, compare, createClient = createShopifyClient } = {}) {
  ...
  return {
    ...
    selling: { type: sell.operation, multiplier: sell.selectedMultiplier, effectiveMultiplier: sell.effective, markupPercent: markupPercent(sell.effective) },
...
export async function applyPriceBatch({ supplierKey, selling, compare, items, createClient = createShopifyClient } = {}) {
  ...
  for (const req of requested) {
    ...
    if (verdict.status === 'already') {
      results.push({ ...base, status: 'already', oldSellingPrice: verdict.liveSelling, newSellingPrice: verdict.targetSelling, oldCompareAtPrice: verdict.liveCompare, wholesale: verdict.wholesale });
      continue;
    }
