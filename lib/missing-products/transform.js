...
export function transformProduct(product, opts) {
  ...
  const isB2bmarkt = config.key === 'b2bmarkt';
  const title = isB2bmarkt ? stripHmCodes(product.title) : (product.title || '');
  const body = isB2bmarkt ? stripHmCodes(product.description) : (product.description || '');
