...
const DEFAULT_MAX_EXPORT_IDENTIFIERS = 5000;
...
export const MAX_EXPORT_IDENTIFIERS = readPositiveIntEnv('MISSING_PRODUCTS_MAX_EXPORT_IDENTIFIERS', DEFAULT_MAX_EXPORT_IDENTIFIERS);
...
export async function listLoadableSuppliers() {
  ...
  for (const key of SUPPLIER_KEYS) {
    ...
    rows.push({
      ...
      available,
      priceMultiplier: adapter.config.priceMultiplier ?? null,
    });
  }
  return rows;
}
...
export async function listSupplierCategories(supplierKey) {
  ...
  const [parsed, labels] = await Promise.all([
    readSupplierFeed(adapter),
    loadCategoryLabels(adapter.config.categoryLabelsPath),
  ]);
  const separator = adapter.config.pathSeparator || ' > ';
  const categories = parsed.categories.map((category) => enrichCategoryLabel(category, labels, separator));
  return {
...
async function loadCategoryLabels(labelsPath) {
  if (!labelsPath) return {};
  try {
    return JSON.parse(await fs.readFile(labelsPath, 'utf8'));
  } catch (e) {
...
/** Split a raw supplier category string into trimmed segments. */
function categorySegments(rawName, separator) {
  return rawName.includes(separator)
    ? rawName.split(separator).map((part) => part.trim()).filter(Boolean)
    : [rawName];
}

/** Bulgarian display label for a raw supplier category (leaf), or the raw leaf. */
function translateCategoryLeaf(rawName, labels, separator) {
  if (!rawName) return '';
  const segments = categorySegments(rawName, separator);
  const leafRaw = segments[segments.length - 1];
  return labels[leafRaw] || labels[rawName] || leafRaw;
}

function enrichCategoryLabel(category, labels, separator) {
  const rawName = category.name;
  const rawSegments = categorySegments(rawName, separator);
  const leafRaw = rawSegments[rawSegments.length - 1];
  const parentRawSegments = rawSegments.slice(0, -1);

  const leafDisplay = labels[leafRaw] || labels[rawName] || leafRaw;
  const parentDisplaySegments = parentRawSegments.map((segment) => labels[segment] || segment);

  const parentPath = parentDisplaySegments.join(' › ');
  const parentPathOriginal = parentRawSegments.join(' › ');

  return {
    ...category,
    displayName: leafDisplay,
    originalName: leafRaw,
    originalPath: rawName,
    parentPath,
    parentPathOriginal,
    hasTranslation: leafDisplay !== leafRaw,
    hasParent: parentDisplaySegments.length > 0,
  };
}
...
function normalizeMissingRow(product, transformResult, validation, categoryName, categoryLabels = {}, separator = ' > ') {
  const firstRow = transformResult.rows[0] || {};
  const rawCategory = firstCategoryName(product, categoryName);
  return {
    id: productIdentifier(product),
    supplierSku: String(product.sku || ''),
    title: firstRow.Title || product.title || '',
    category: rawCategory,
    categoryDisplay: translateCategoryLeaf(rawCategory, categoryLabels, separator),
...
export async function scanMissingProducts({
  ...
  const labels = await loadCategoryLabels(adapter.config.categoryLabelsPath);
  const separator = adapter.config.pathSeparator || ' > ';
  ...
  const missingProducts = comparison.missing.map((product) => {
    ...
    return normalizeMissingRow(product, transformed, validateSummary(summary), category.name, labels, separator);
  });

  const enrichedCategory = enrichCategoryLabel(category, labels, separator);
  ...
  return {
    ...
    category: {
      id: category.id,
      name: category.name,
      displayName: enrichedCategory.displayName,
      originalName: enrichedCategory.originalName,
      originalPath: enrichedCategory.originalPath,
      parentPath: enrichedCategory.parentPath,
      hasTranslation: enrichedCategory.hasTranslation,
      hasParent: enrichedCategory.hasParent,
    },
    totals: {
...
function validateExportIdentifiers(productIds) {
  ...
  if (productIds.length > MAX_EXPORT_IDENTIFIERS) {
    const error = new Error(`Too many selected products. Maximum is ${MAX_EXPORT_IDENTIFIERS}. Export fewer products or raise MISSING_PRODUCTS_MAX_EXPORT_IDENTIFIERS for this environment.`);
    error.statusCode = 400;
    throw error;
  }
