...
import {
  ...
  listLoadableSuppliers,
  listSupplierCategories,
  scanMissingProducts,
} from './missing-products/dashboard.js';
import {
  getPriceSupplierInfo,
  previewPrices,
  applyPriceBatch,
  rollbackPriceBatch,
} from './prices/dashboard.js';
...
function sendCsv(res, filename, csv, summary) {
  res.writeHead(200, {
    'Content-Type': 'text/csv; charset=utf-8',
    'Content-Disposition': `attachment; filename="${filename}"`,
    'Cache-Control': 'no-store',
    'X-Mebelcenter-Export-Summary': Buffer.from(JSON.stringify(summary), 'utf8').toString('base64url'),
  });
  res.end(csv);
}

async function sendAppShell(res, statusCode = 200) {
  await sendStaticFile(res, path.resolve('public', 'app', 'index.html'), {
    statusCode,
...
async function sendStaticFile(res, filePath, opts = {}) {
  try {
    const body = await fs.readFile(filePath);
    res.writeHead(opts.statusCode || 200, {
    ...
    if (error.code === 'ENOENT') {
      sendJson(res, 404, { error: 'Static asset not found. Run npm run build.' });
      return;
    }
    throw error;
  }
...
async function handleLogin(req, res) {
  if (!getDashboardPassword()) {
    if (wantsJson(req)) sendJson(res, 500, { error: 'DASHBOARD_PASSWORD is not configured.' });
    else sendHtml(res, 500, renderLoginPage('DASHBOARD_PASSWORD is not configured.'));
    return;
  }
  ...
  if (password !== envPassword) {
    if (wantsJson(req)) sendJson(res, 401, { error: 'Невалидна парола.' });
    else sendHtml(res, 401, renderLoginPage('Invalid password.'));
    return;
  }
  ...
  if (wantsJson(req)) {
    ...
    redirect(res, '/dashboard', headers);
  }
}
...
async function handleMissingSuppliers(req, res) {
  if (!requireAuth(req, res)) return;
  try {
    sendJson(res, 200, { suppliers: await listLoadableSuppliers() });
  } catch (error) {
    sendJson(res, error.statusCode || 500, { error: error.message });
  }
}

async function handleMissingCategories(req, res, url) {
  if (!requireAuth(req, res)) return;
  try {
    const supplier = url.searchParams.get('supplier');
    const result = await listSupplierCategories(supplier);
...
async function handleMissingScan(req, res) {
  if (!requireAuth(req, res)) return;
  let payload;
  try {
    payload = JSON.parse(await readRequestBody(req));
...
async function handleMissingExport(req, res) {
  if (!requireAuth(req, res)) return;
  const rawBody = await readRequestBody(req);
  if (rawBody.length > 1_000_000) {
    sendJson(res, 413, { error: 'Export request is too large.' });
    return;
  }
  ...
  try {
    ...
    sendCsv(res, result.filename, result.csv, result.summary);
  } catch (error) {
...
async function readJsonBody(req, res, maxBytes = 512_000) {
  const rawBody = await readRequestBody(req);
  if (rawBody.length > maxBytes) {
    sendJson(res, 413, { error: 'Заявката е твърде голяма.' });
    return { ok: false };
  }
  try {
    return { ok: true, payload: rawBody ? JSON.parse(rawBody) : {} };
  } catch {
    sendJson(res, 400, { error: 'Тялото на заявката трябва да е валиден JSON.' });
    return { ok: false };
  }
}

async function handlePriceSuppliers(req, res) {
  if (!requireAuth(req, res)) return;
  try {
    sendJson(res, 200, { suppliers: getPriceSupplierInfo() });
  } catch (error) {
    sendJson(res, error.statusCode || 500, { error: error.message });
  }
}

// READ-ONLY: never mutates Shopify.
async function handlePricePreview(req, res) {
  if (!requireAuth(req, res)) return;
  const body = await readJsonBody(req, res);
  if (!body.ok) return;
  try {
    const result = await previewPrices({
      supplierKey: body.payload?.supplier,
      selling: body.payload?.selling,
      compare: body.payload?.compare,
    });
    sendJson(res, 200, result);
  } catch (error) {
    sendJson(res, error.statusCode || 500, { error: error.message });
...
// Writes ONE small browser-driven batch. Server re-verifies + recomputes.
async function handlePriceApplyBatch(req, res) {
  if (!requireAuth(req, res)) return;
  const body = await readJsonBody(req, res);
  if (!body.ok) return;
  try {
...
async function handlePriceRollbackBatch(req, res) {
  if (!requireAuth(req, res)) return;
  const body = await readJsonBody(req, res);
  if (!body.ok) return;
  try {
    const result = await rollbackPriceBatch({ supplierKey: body.payload?.supplier, items: body.payload?.items });
    sendJson(res, 200, result);
  } catch (error) {
    sendJson(res, error.statusCode || 500, { error: error.message });
  }
...
export async function handleDashboardRequest(req, res) {
  ...
  if (req.method === 'GET' && url.pathname.startsWith('/app/')) {
    const relPath = decodeURIComponent(url.pathname.slice('/app/'.length));
    const root = path.resolve('public', 'app');
    if (!relPath || !path.extname(relPath)) {
      await sendAppShell(res, 200);
      return;
    }
    const filePath = path.resolve(root, relPath || 'index.html');
  ...
  if (
    ...
    (url.pathname === '/dashboard' ||
      url.pathname === '/missing-products' ||
      url.pathname === '/inventory' ||
      url.pathname === '/prices')
  ) {
    if (!getDashboardPassword()) {
      await sendAppShell(res, 500);
      return;
    }
    if (!isAuthenticated(req)) {
      redirect(res, '/');
  ...
  if (req.method === 'GET' && url.pathname === '/api/missing-products/suppliers') {
    await handleMissingSuppliers(req, res);
    return;
  }

  if (req.method === 'GET' && url.pathname === '/api/missing-products/categories') {
    await handleMissingCategories(req, res, url);
    return;
  }

  if (req.method === 'POST' && url.pathname === '/api/missing-products/scan') {
    await handleMissingScan(req, res);
    return;
  }

  if (req.method === 'POST' && url.pathname === '/api/missing-products/export') {
    await handleMissingExport(req, res);
    return;
  }

  if (req.method === 'GET' && url.pathname === '/api/prices/suppliers') {
    await handlePriceSuppliers(req, res);
    return;
  }

  if (req.method === 'POST' && url.pathname === '/api/prices/preview') {
    await handlePricePreview(req, res);
    return;
  }

  if (req.method === 'POST' && url.pathname === '/api/prices/apply-batch') {
    await handlePriceApplyBatch(req, res);
    return;
  }

  if (req.method === 'POST' && url.pathname === '/api/prices/rollback-batch') {
    await handlePriceRollbackBatch(req, res);
    return;
  }
