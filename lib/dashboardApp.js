  ...
  <body>
    <div class="app-shell">
      ...
      <main class="main">
        ...
        <section class="supplier-grid" aria-label="Shopify inventory suppliers">
          ${renderSupplierCard(SUPPLIERS.megapap)}
          ${renderSupplierCard(SUPPLIERS.b2bmarkt)}
        </section>

        <section class="b2bcenter-section" aria-label="B2BCenter portal sync">
          <div class="section-heading">
            <p class="eyebrow">Supabase Portal</p>
            <h2>B2BCenter Portal Sync</h2>
            <p class="section-note">Dry-run only. Reads the B2BCenter Supabase <code>products</code> table (stock/quantity, SKU-matched, tenant-scoped). This does <strong>not</strong> touch Shopify and performs <strong>no database writes</strong>. Apply mode is a future phase.</p>
          </div>
...
function renderSupplierCard(supplier) {
  return `<article class="supplier-card ${supplier.tone}" data-card="${supplier.key}">
    ...
    <div class="button-row">
      <button class="run-button safe" type="button" data-target="shopify" data-supplier="${supplier.key}" data-dry-run="true">Dry Run</button>
      <button class="run-button danger" type="button" data-target="shopify" data-supplier="${supplier.key}" data-dry-run="false">Apply Sync</button>
    </div>
  </article>`;
}

function renderB2BCenterCard(supplier) {
  return `<article class="supplier-card violet" data-card="b2bcenter-${supplier.key}">
    <div class="card-top">
      <div>
        <span class="supplier-kicker">B2BCenter · ${supplier.manufacturer}</span>
        <h2>${supplier.name}</h2>
      </div>
      <span class="health-dot dry">Dry-run</span>
    </div>
    <div class="supplier-meta">
      <span>Supabase</span>
      <span>Stock only</span>
      <span>SKU match</span>
      <span>Dry-run only</span>
    </div>
    <div class="last-result" id="last-b2bcenter-${supplier.key}">Last result: waiting for run data</div>
    <div class="button-row">
      <button class="run-button safe" type="button" data-target="b2bcenter" data-supplier="${supplier.key}" data-dry-run="true">Dry Run</button>
      <button class="run-button apply-disabled" type="button" disabled title="B2BCenter apply mode is not implemented yet">Apply later</button>
    </div>
  </article>`;
}
...
function renderClientScript() {
  return `
    ...
    const runButtons = Array.from(document.querySelectorAll('.run-button[data-supplier]'));
    ...
    async function runSync(supplierKey, dryRun, target) {
      ...
      // B2BCenter is dry-run only; never send a non-dry-run request for it.
      const effectiveDryRun = runTarget === 'b2bcenter' ? true : dryRun;
      if (runTarget === 'shopify' && !effectiveDryRun) {
      ...
      const label = (runTarget === 'b2bcenter' ? 'B2BCenter ' : '') + supplierKey;
      localRunning = true;
      setButtonsDisabled(true);
      setStatus('Running ' + label, 'running');
      setSummary(null);
      setLogs(['Starting ' + label + ' ' + (effectiveDryRun ? 'dry run' : 'apply sync') + '...']);
      ...
      try {
        const res = await fetch('/api/sync', {
          ...
          body: JSON.stringify({ target: runTarget, supplierKey, dryRun: effectiveDryRun }),
        });
        const data = await res.json();
        if (!res.ok) {
...
async function handleSync(req, res) {
  ...
  if (target === 'b2bcenter') {
    ...
    if (!dryRun) {
      sendJson(res, 400, {
        error: 'B2BCenter apply mode is not implemented yet. Run dry-run only.',
      });
      return;
    }
  ...
  try {
    ...
    const result =
      target === 'b2bcenter'
        ? await runB2BCenterSync({ supplierKey, dryRun: true, onLog })
