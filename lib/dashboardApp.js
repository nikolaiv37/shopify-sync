...
function renderB2BCenterCard(supplier) {
  return `<article class="supplier-card violet" data-card="b2bcenter-${supplier.key}">
    <div class="card-top">
      ...
      <span class="health-dot dry">Ready</span>
    </div>
    <div class="supplier-meta">
      <span>Supabase</span>
    ...
    <div class="button-row">
      ...
      <button class="run-button warn" type="button" data-target="b2bcenter" data-supplier="${supplier.key}" data-dry-run="false">Apply Sync</button>
    </div>
  </article>`;
}
  ...
  return `
    ...
    async function runSync(supplierKey, dryRun, target) {
      ...
      if (runTarget === 'b2bcenter' && isApply) {
        const names = { megapap: 'Megapap/Mebelcenter', b2bmarkt: 'B2BMarkt/Europe' };
        const confirmed = window.confirm(
          'Apply B2BCenter stock sync for ' + (names[supplierKey] || supplierKey) + '?\\n\\n' +
          'This will update product quantity values in the B2BCenter portal.\\n' +
          'It will NOT update prices, categories, names, images, or visibility.\\n\\n' +
          'Run a dry-run first and confirm the planned changes are expected.'
        );
        if (!confirmed) return;
      }
      ...
      setSummary(null);
      setLogs(['Starting ' + label + ' ' + (isApply ? 'apply sync' : 'dry run') + '...']);
